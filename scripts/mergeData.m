...
for ivomd = 2:length(varsOfMetadata)
    cvomd = varsOfMetadata{ivomd};
...
for iusr = 1:nusr
    ...
    if height(curUsrMetadata) > 1 %Mutiple entries for current user's basic information.
        ...
        for ivomd = 2:length(varsOfMetadata)
            cvomd = varsOfMetadata{ivomd};
            if ~all(isundefined(curUsrMetadata.(cvomd))) && ... %All of the information is undefined.
                    length(unique(curUsrMetadata.(cvomd))) ~= 1 %Only one category is found.
                mrgResolved = false;
            end
        end
        if mrgResolved
